...
// ၁။ ဝတ္ထုအားလုံးကို ဆွဲထုတ်မည့် Function (Homepage အတွက်) Pagination ပါဝင်သည်
...
export const getTotalNovelsCount = async (db: DrizzleD1Database<Record<string, unknown>>) => {
...
export const getTopNovelsByViews = async (db: DrizzleD1Database<Record<string, unknown>>, limit: number = 3) => {
  return await db
    .select()
    .from(novels)
...
export const getNovelBySlug = async (db: DrizzleD1Database<Record<string, unknown>>, slug: string) => {
  return await db
