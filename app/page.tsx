    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--background)] text-[var(--foreground)] p-4">
        <div className="max-w-md w-full bg-[var(--surface)] border border-red-500/50 rounded-2xl p-6 text-center space-y-4 shadow-xl">
          ...
          <h1 className="text-xl font-bold text-[var(--foreground)]">Database Connection Error</h1>
          <p className="text-sm text-[var(--text-muted)]">
            Cloudflare D1 Database binding <strong>(DB)</strong> is missing or misconfigured in your Pages deployment settings.
          </p>
          <p className="text-xs text-[var(--text-muted)] pt-2 border-t border-[var(--border)]">
            Please go to Cloudflare Dashboard &gt; Workers &amp; Pages &gt; pyunovel &gt; Settings &gt; Bindings and ensure you have added a D1 database binding named exactly <strong>DB</strong>.
          </p>
        </div>
      </div>
    );
