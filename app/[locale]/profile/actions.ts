...
export async function claimDailyCheckIn() {
    ...
    if (user.lastCheckIn) {
        ...
        var newStreak = isConsecutive ? (user.checkInStreak ?? 0) + 1 : 1;
    } else {
        var newStreak = 1;
    }

    // Tiered daily EXP cap based on streak milestone
    // Streak < 30:   max 30 EXP/day  (base 10 + up to 20 bonus)
    // Streak 30-89:  max 40 EXP/day  (base 10 + up to 30 bonus)
    // Streak 90-364: max 60 EXP/day  (base 10 + up to 50 bonus)
    // Streak 365+:   max 100 EXP/day (base 10 + up to 90 bonus)
    let maxBonus: number;
    if (newStreak >= 365) {
        maxBonus = 90;
